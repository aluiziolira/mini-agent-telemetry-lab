...
if not ingest_api_key:
    raise RuntimeError("INGEST_API_KEY is required to run scripts/demo_agent_msft.py")

llm_api_key = os.environ.get("LLM_API_KEY")
if not llm_api_key:
    raise RuntimeError("LLM_API_KEY is required to run scripts/demo_agent_msft.py")

tracer = Tracer(
    os.environ.get("TELEMETRY_BASE_URL", "http://127.0.0.1:8000"),
    ingest_api_key,
)
tracer.agent_name = "research_analyst_msft"
...
def extract_symbol(query):
    ...
    return matches[-1] if matches else "MSFT"
...
if __name__ == "__main__":
    user_query = os.sys.argv[1] if len(os.sys.argv) > 1 else "Should I buy MSFT?"
    symbol = extract_symbol(user_query)
    with tracer.span("research_analyst_msft_run", "chain") as root_span:
        ...
        with tracer.span("synthesis_call", "llm", parent_span_id=root_span.span_id) as llm_span:
            prompt_body = (
                ...
                "Provide a concise grounded recommendation focused on Microsoft."
            )
            prompt = truncate(
                prompt_body,
                2000,
